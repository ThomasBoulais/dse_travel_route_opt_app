...
def get_pois() -> gpd.GeoDataFrame:
    ...
    log.info(f"OSM - Téléchargement des POIs OSM de: {OSM_PLACE_NAME}.")
...
def get_road_networks() -> gpd.GeoDataFrame:
    ...
    log.info(f"OSM - Téléchargement du réseau de routes de: {OSM_PLACE_NAME}")
...
def ingest_bronze(pois_gdf: gpd.GeoDataFrame, G: gpd.GeoDataFrame) -> None:
    ...
    log.info("OSM - GeoParquet des POIs bruts sauvegardés à l'endroit suivant: data/bronze/osm/osm_pois.geoparquet")
    ...
    log.info(f"OSM - Graphml des réseaux de routes sauvegardés à l'endroit suivant: {OSM_BRONZE_GRAPHML}")
