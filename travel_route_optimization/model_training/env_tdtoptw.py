...
class TDTOPTWEnv:
    ...
    def _feasible_actions_mask(self) -> np.ndarray:
        ...
        for a in range(self.max_actions):
            ...
            # Only check opening at arrival time
    ...
    def step(self, action_idx: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        # If we visit an accommodation
        if self.is_accommodation[next_poi]:
            self.accommodation_used_today = True

        if self.is_accommodation[next_poi] and self.current_minute < 18*60:
            reward -= 1.0
        ...
        route_length_bonus = 0.5 * visited_count  # small but cumulative
        reward += route_length_bonus
